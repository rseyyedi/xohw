...
static uint16_t device_id_pl_data = XPAR_FLIGHT_CONTROLLER_AXI_BRAM_CTRL_FC_TO_SA_DEVICE_ID;

/*!
 * These function initialize the dataminer bram and set some default values to the data registers
 *
...
bram_error_e bram_pl_data_init(void)
{
	 ...
	 */
	for (i = 0; i < (config_ptr_pl_data->MemHighAddress - config_ptr_pl_data->MemBaseAddress); i+=4) // TODO!!!!!!!! is +4 really correct here? without OVP tells us a misaligned error
	{
